...
for i, (indices, data) in enumerate(zip(data_indices, datasets)):
    ...
    plt.scatter(indices, data, label=f'Distance = {labels[i]}', color=colors[i], s=10)

    # Add mean value as a horizontal line within the range of the dataset
    mean_value = np.mean(data)
    plt.plot(indices, [mean_value] * len(indices), color=colors[i], linestyle='--', linewidth=2, label=f'Distance = {labels[i]} (Mean)')
    ...
    plt.plot(indices, fit_line, color=colors[i], linestyle=':', linewidth=2, label=f'Distance = {labels[i]} (Linear Fit)')

# Customize the plot
plt.title('ToF different distances to mirror')
plt.xlabel('Index')
plt.ylabel('ToF [ps]')
