...
\subsubsection{Weitere Annäherung der Berechnung mit Empfangs-Linse}
...
Ein Mikrocontroller kümmert sich um das Starten und Auswerten der Messresultate. Das Generieren eines Startsignals hat
...
Lichtstrom umgewandelt. Dieser Lichtstrom wird von einem Transimpedanzverstärker in eine Spannung umgewandelt. Bevor der
Mikrokontroller diese Spannung auswerten kann, muss diese erst digitalisiert werden. Das passiert unter Verwendung eines Komparators.
Dieser kann als 1 bit \acrshort{adc} verstanden werden, da bloss eine Unterscheidung der beiden Zustände \glqq Licht\grqq und
\glqq kein Licht\grqq notwendigt ist. Es ergibt sich also ein Empfangspuls.
...
Zeitmessung. Die Firma Texas Instruments bietet dafür mit dem \acrshort{tdc}7200 eine gute Lösung
...
zeitliche Auflösung in eine Bewegungsgleichung ein, sollte der Baustein theoretisch folgende
räumliche Auflösung erreichen:

\begin{equation}\label{eq:tdc_max_resolution}
        s_{min} = v \cdot t = 299.8e6 \frac{m}{s} \cdot 55ps = 1.5mm
\end{equation}
...
In einem ersten Teil soll es rein darum gehen, den \acrshort{tdc}7200 \acrshort{ic} genauer
kennenzulernen.
...
die Schaltzeit des Komparators eine grosse Rolle. Auch der Jitter des Mikrokontrollers beim Ein- und Ausschalten
seiner Ausgänge wird bei der minimalen Auflösung ins Gewicht fallen. Im Kapitel \textcolor{red}{TODO Verweis}
wird mittels verschiedener Messungen dargestellt, wie nah an die vom TDC7200 vorgegebene Grenze rangekommen wird.

\subsection{Vorangehensweise}
...
\begin{figure}[H]
    ...
    \caption{Konzept Betrieb TDC rein elektrisch}\label{fig:konzept_tdc_electrical}
\end{figure}
...
\begin{itemize}
    \item Messung Jitter der Ausgänge des verwendeten Mikrocontrollers
...
Nachfolgend werden sämtliche Teil-Schaltungen thematisiert, welche für das entwickelten ToF-Evaluationsmodul
...
Designt wurde das Schema mit Hilfe des Open-Source Tools \glqq KiCad EDA 8.0\grqq.
...
Das NUCLEO Board ist ein sogenanntes \glqq Development-Kit\grqq, welches einen STM32F042K6 Mikrocontroller
...
enorm. Programmiert wird der Mikrokontroller über eine USB-Schnittstelle.
...
und auch auszulesen (siehe SPI-Bus in der Abbildung). Weiter können auf beiden TDCs Messungen
gestartet werden mit den Signalen \glqq start\_ele\grqq, resp. \glqq start\_opt\grqq. Für den
TDC welcher sich um die elektrischen Signale kümmert kann zudem mit \glqq stop\_ele\grqq ein
Stopp-Puls generiert werden.
Zuguterletzt ist das NUCLEO dafür zuständig, die Laser-Diode anzusteuern, was mit den Signalen
\glqq LD\_pulse\grqq sowie  \glqq $\overline{\mbox{LD\_enable}}$\grqq geschieht.
...
\begin{equation}\label{eq:mode1_vs_mode2}
    \begin{split}
        \Delta Mode~2 &= 1'403'223.8~ps - 1'378'222.5~ps = 25'001.3~ps\\
        Mode~1        &= 25'145.3~ps
    \end{split}
\end{equation}
