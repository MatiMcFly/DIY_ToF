...
Zusätzlich werden die verschiedenen Kabellängen gleichzeit auch via \acrshort{dso} (Rohde \& Schwarz RTB2004 2.5 GSa/s.)
...
erfassten Messresultate des \acrshort{tdc} aufgelistet, während dem der \acrshort{dso} gleichzeitig an den Pins
angeschlossen war. Die Datensets befinden sich im elektronischen Anhang.
...
aufgelistet. Die letzte Spalte zeigt den Unterschied zu den Messresultaten des \acrshort{tdc}. Es ist zu erkennen, dass
der Unterschied sehr klein ist.
...
unterschiedlichen Annahmen für die Signalausbreitungs-Geschwindigkeit von $\sfrac{2}{3}~c$ und $c$.
...
Der Unterschied der \acrshort{tdc}-Resultate zu den theoretischen Erwartungen sind in
Tabelle~\ref{tab:different_cable_lengths_with_dso_theorie_vs_tdc_ns} und
Tabelle~\ref{tab:different_cable_lengths_with_dso_theorie_vs_tdc_m} dargestellt. Zuerst im Bezug auf den
Laufzeit-Unterschied, dann die zurückgerechnete Kabellänge.
...
Kapitel~\ref{sec:ausblick}. %TODO Fazit/Ausblick oder so?
...
%TODO Mögliche Gründe für diesen Unterschied: Weniger steile Flanke, wenn weiter weg? Time-Drift?
...
Mögliche Verbesserungen dazu sind im Kapitel~\ref{sec:ausblick} aufgeführt. %TODO Fazit/Ausblick oder so?
